...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    /* ── GSAP scroll story — inside a context so React StrictMode's double
       ...
       whole page invisible: the 2nd `from()` captured opacity 0 as the end
       state). ctx.revert() restores pristine styles between runs. ── */
    if (!reduced) {
      ...
      const gctx = gsap.context(() => {
        // fromTo (not from): explicit end values mean a re-run can never
        // capture a mid-animation opacity-0 state as the destination.
        gsap.fromTo("#heroTitle .ch",
          { yPercent: 115, opacity: 0, rotateX: -55 },
          { yPercent: 0, opacity: 1, rotateX: 0, stagger: 0.02,
            duration: 0.85, ease: "back.out(1.5)", delay: 0.15 });
        gsap.fromTo(".landing .eyebrow,.landing .hero-sub,.landing .hero-cta,.landing .hero-agents",
          { y: 24, opacity: 0 },
          { y: 0, opacity: 1, stagger: 0.1, duration: 0.75, ease: "power3.out", delay: 0.5 });
        gsap.to("#tlFill", {
